// Operates NanoUI
/obj/item/modular_computer/ui_interact(mob/user, ui_key = "main", datum/tgui/ui = null, var/force_open = TRUE, datum/tgui/master_ui = null, datum/ui_state/state = GLOB.default_state)
	if(!enabled)
		if(ui)
			ui.close()
		return 0
	...
	if(!ui)
		...
		ui = new(user, src, ui_key, "Computer_Main", name, 300, 300, master_ui, state)
